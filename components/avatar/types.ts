...
export type AvatarMuscleDatum = {
  ...
  weeklyVolume: number[];
  /** H18.2 — целевая частота (working-подходов/нед) для этой группы, если у
   *  атлета задана цель из дрилла мышцы; иначе null (трек-бар не рендерится).
   *  Только свой /profile передаёт цели — у друга/на дашборде всегда null. */
