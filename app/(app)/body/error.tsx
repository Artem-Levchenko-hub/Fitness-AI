...
/** Error-состояние экрана «Тело» (R-37). Ловит сбой загрузки профиля/замеров
 *  (например, недоступность БД при getUserProfile/listMeasurements) и даёт
 *  повтор без перезагрузки страницы. Next 16: проп называется `unstable_retry`,
 *  НЕ `reset`. */
export default function BodyError({
  ...
  useEffect(() => {
    ...
    console.error("body load failed", error);
  }, [error]);
      ...
      <div className="bg-card border-border space-y-4 rounded-2xl border p-8 text-center">
        ...
        <div>
          <p className="text-foreground text-base font-medium">
            Не удалось загрузить данные о теле
          </p>
          <p className="text-muted-foreground mt-1 text-sm leading-relaxed">
            Что-то пошло не так при загрузке замеров и профиля. Попробуйте ещё
            раз — ваши замеры на месте.
          </p>
        </div>
        <div className="mt-2">
