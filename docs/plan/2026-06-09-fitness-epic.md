...
- **NEXT ACTION (продолжить отсюда):** **F8 run-2 — клиент-консьюмер стрима.** run-1 (endpoint `POST /api/ai/trainer/stream`) ЗАДЕПЛОЕН+ПРОВЕРЕН на проде (master `2da7220`): стримит structured JSON, `onFinish` сохраняет `ai_analyses` с цветными дельтами. Осталось: на `/workouts/[id]/trainer` для on_demand-захода заменить `TrainerJobPoller` на стрим-консьюмер (см. чеклист F8 run-2 — план готов, включая дизайн-нюанс «не рендерить сырой JSON, показывать прогресс → перечитать ai_analyses»). Poll оставить fallback. **Context7 (`@ai-sdk/react` / fetch ReadableStream) + sequential-thinking обязательно.**
...
- [ ] **run-2: клиент-консьюмер** заменяет `TrainerJobPoller` на on_demand-путь: на `/workouts/[id]/trainer` без готового анализа — fetch `POST /api/ai/trainer/stream`, читать text-stream (live «тренер анализирует…» / прогресс), по завершении → перечитать сохранённый анализ и показать `TrainerResultCard` (цветные дельты). Poll-путь (cron + poller) оставить fallback для push/повторного захода. **Дизайн-нюанс:** стрим эмитит сырой JSON (а у thinking-модели — reasoning+JSON), не pretty markdown → клиент НЕ рендерит сырые токены как текст, а показывает индикатор прогресса по факту поступления байт, затем читает `ai_analyses` (через server action / refetch jobs-style endpoint). Решить: после стрима как получить сохранённый resultJson — (a) повторный GET последнего `ai_analyses` по workoutId (новый маленький endpoint/action), или (b) парсить JSON на клиенте из накопленного стрима. (a) надёжнее (та же Zod-валидация на сервере).
