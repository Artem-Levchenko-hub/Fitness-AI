...
- **NEXT ACTION (продолжить отсюда):** **F8 — стриминг разбора.** Боль владельца: «разбор в реальном времени, не перезаходить». Сейчас trainer = poll (`components/trainer/TrainerJobPoller.tsx` поллит `/api/ai/jobs/[id]` каждые 2с). Сделать LIVE-стрим генерации на `/workouts/[id]/trainer` по образцу coach-route (`app/api/ai/coach/route.ts` → `streamText` + `toTextStreamResponse`); результат сохранять в `ai_analyses` в конце стрима. Структурный JSON стримить сложнее — вариант: стримить markdown-разбор live + параллельно/после досчитывать structured для дельт. **Context7 (Vercel AI SDK `streamText`/`useChat`) + sequential-thinking обязательно.**
