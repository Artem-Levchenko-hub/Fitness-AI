...
- [ ] **run-3b: cron-доставка push-напоминаний** — `push_kind`+=`workout_reminder` (миграция psql) + ветка в `push-reminders` cron (день+час+tz match, дедуп через notifications_log) → `sendPushToUser` url `/create`
