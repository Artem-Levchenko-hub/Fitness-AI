...
`BILLING_ENABLED` и `SUBSCRIPTION_ENABLED` включаются только после добавления
боевых ключей, реквизитов продавца, подтверждённой оферты и чеков. Пока
`YOOKASSA_RECURRING_ENABLED=false`, подписка продаётся как разовый доступ на
выбранный период без сохранения способа оплаты. Автопродление включается только
после подтверждения прав менеджером ЮKassa.
