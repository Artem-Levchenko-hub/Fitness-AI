...
try {
  if (process.argv[2] === "--cleanup") {
    ...
    const resultJson = {
      ...
      // H13.2/H13.3: оба фактора учтены (score != null) → на своём разборе
      // заголовки кликабельны: «Восстановление (сон)» → /sleep, «Питание (КБЖУ)»
      // → /nutrition (граница «нет данных → статика» покрыта юнит-тестом
      // resolveLifeFactorHref: score null → null).
      ...
      // H17.0-B: тренер назвал группы баланса прозой + структурными ключами →
      // под секцией «Баланс по аватару» рендерится кликабельный силуэт, тап по
      // группе → /profile?muscle=<key> (deep-link sub-slice A).
      muscleBalanceNote:
        "Грудь перегрета (16 подходов), квадрицепс холодный (0) — добавь присед 3×8.",
      balanceMuscleKeys: ["chest", "quads"],
    };
