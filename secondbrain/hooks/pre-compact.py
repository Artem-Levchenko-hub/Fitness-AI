"""Cursor preCompact hook: spawn background flush before compaction."""
...
def _spawn_flush(payload_file: Path, payload: dict) -> None:
    cmd = [sys.executable, str(FLUSH_SCRIPT), "--payload-file", str(payload_file), "--event", "preCompact"]
    env = os.environ.copy()
    ...
    err_log = open(log_dir / "flush-pre-compact.err.log", "a", encoding="utf-8")
...
def main() -> int:
    ...
    payload_file = TMP_DIR / f"pre-compact-{os.getpid()}.json"
